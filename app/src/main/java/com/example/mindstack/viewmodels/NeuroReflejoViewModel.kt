...
class NeuroReflejoViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun finishAndSubmit(authVm: AuthViewModel, checkinId: Int, onComplete: () -> Unit) {
        if (checkinId == 0) {
            Log.e("NEURO_SYNC", "No se puede enviar: checkinId es 0")
            isGameOver = true
            isGameRunning = false
            onComplete()
            return
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                Log.d("NEURO_SYNC", "Enviando resultados: $request")
                val response = RetrofitClient.gameService.submitNeuroReflex(token, request)
                
                if (response.isSuccessful) {
                    Log.d("NEURO_SYNC", "Resultados guardados correctamente: ${response.body()}")
                } else {
                    Log.e("NEURO_SYNC", "Error al guardar: ${response.code()} - ${response.errorBody()?.string()}")
                }
            } catch (e: Exception) {
                Log.e("NEURO_SYNC", "Fallo de conexión: ${e.message}")
            } finally {
                isSending = false
                onComplete()
            }
